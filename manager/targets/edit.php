...
if (!$belongs_to_manager) {
    $_SESSION['error_message'] = 'ليس لديك صلاحية تعديل هذا الهدف';
    header('Location: index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // التحقق من عدم وجود تداخل في الفترات (مع استثناء الهدف الحالي)
    ...
    if (empty($salesperson_id_err) && empty($target_amount_err) && empty($start_date_err) && empty($end_date_err)) {
        ...
        // الاحتفاظ بالمبلغ المحقق كما هو
        ...
        <div class="card-body">
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $target_id; ?>">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="achieved_amount" class="form-label">المبلغ المحقق حتى الآن</label>
                        <div class="input-group">
                            <input type="number" step="0.01" min="0" class="form-control" id="achieved_amount" value="<?php echo $achieved_amount; ?>" readonly>
                            <span class="input-group-text">ج.م</span>
                        </div>
                        <small class="text-muted">يتم تحديث هذه القيمة تلقائيًا عند تسجيل المبيعات</small>
                    </div>
                </div>
                
                <!-- نسبة التحقق -->
                <div class="mb-3">
                    <label class="form-label">نسبة تحقيق الهدف</label>
                    <?php
                        $achievement_percentage = calculateAchievement($achieved_amount, $target_amount);
